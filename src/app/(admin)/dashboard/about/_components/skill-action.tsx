...
export const SkillAction = ({ data, fetchData }: SkillActionProps) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`/api/skills/${encodeURIComponent(publicId)}`);
  ...
  const handleUpdate = async ({ image, title }: FormImageUploadValues) => {
    try {
      ...
      const response = await axios.put(`/api/skills/${encodeURIComponent(publicId)}`, formData);
    ...
    <Popover open={openPopover} onOpenChange={setOpenPopover}>
      <PopoverTrigger asChild>
        <Button variant='ghost' className='h-auto w-auto p-1'>
          <MoreVertical className='size-4 opacity-75' />
        </Button>
      </PopoverTrigger>
